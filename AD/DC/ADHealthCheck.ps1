...
# If you get an error installing PSwrite run this Error fix .... [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
#
#############################    EMAIL Loop   ################################
        $From = "Email from"
        $To = "Email to"  
        $Date = (Get-Date -UFormat "%A %m/%d/%Y %R %Z")
        $subject = "Active Directory Health Summary"
...
$dcs = $isup
$session = New-PSSession  -ComputerName $dcs -EnableNetworkAccess
...
$res = Invoke-Command -Session $session -ScriptBlock{
        ...
        ####################SystemLog status##################
         ...
         ####################Boot Time#########################
         $lastReboot = (Get-Date) - ((Get-WmiObject win32_operatingsystem).ConvertToDateTime((Get-WmiObject win32_operatingsystem).lastbootuptime)) | select -ExpandProperty Days
         ######################################################

        New-Object PSObject -Property ([ordered]@{
            Identity          = $ENV:COMPUTERNAME
            "Days Since Last Reboot"    = $lastReboot
...
$res | ft
$data = $res|sort Identity|Select-Object "Identity","Days Since Last Reboot","NetlogonService","NTDSService","DNSServiceStatus","Netlogons Test","Replication Test","Services Test","Advertising Test","Intersite Test","kCC Test","Topology Test","FSMO Test"
...
Email {
    ...
    EmailBody -FontFamily 'Calibri' -Size 12 {
        ...
        EmailTable -DataTable $data {
          EmailTableHeader -Names 'Days Since Last Reboot','NetlogonService','NTDSService', 'DNSServiceStatus' -Title 'Services' -Alignment center -Color White -BackGroundColor Gray
          EmailTableHeader -Names "Netlogons Test","Replication Test","Services Test","Advertising Test","Intersite Test","kCC Test","Topology Test","FSMO Test" -Title 'dcdiag test results' -Alignment center -Color White -BackGroundColor Gray
           
            
            EmailTableCondition -ComparisonType 'number' -Name 'Days Since Last Reboot' -Operator lt -Value 60 -BackgroundColor MoonYellow -Color Black -Inline
            EmailTableCondition -ComparisonType 'number' -Name 'Days Since Last Reboot' -Operator lt -Value 30 -BackgroundColor Green -Color White -Inline
            EmailTableCondition -ComparisonType 'number' -Name 'Days Since Last Reboot' -Operator gt -Value 60 -BackgroundColor BrickRed -Color White -Inline -Row
