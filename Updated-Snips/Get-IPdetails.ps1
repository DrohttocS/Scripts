...
$Computers =  Get-ADComputer -Properties * -Filter {enabled -eq $true -and OperatingSystem -like  "*server*"} 
$Computers = $Computers.name
cls
$Computers.Count


 $IPdetails = foreach ($Computer in $Computers) {
  if(Test-Connection -ComputerName $Computer -Count 1 -ea 0) {
   try {
    $Networks = Get-WmiObject Win32_NetworkAdapterConfiguration -ComputerName $Computer -EA Stop -Credential $AdminCred | ? {$_.IPEnabled}
   } catch {
...
#    $WINS = @($WINS1,$WINS2)         
 ...
 $IPdetails | sort computername | Export-Csv -Path C:\Temp\IPAddress.csv -NoTypeInformation 
