...
$res = Invoke-Command -Session $session {
Function Get-PendingRebootStatus {
<#
.Synopsis
    This will check to see if a server or computer has a reboot pending.
...
Get-PendingRebootStatus} -ErrorAction SilentlyContinue
$session | Remove-PSSession

$res |select Computername, Pendingreboot | sort Pendingreboot,Computername |  ft
$Need2Boot = $res | ? {$_.pendingreboot -eq $true}  | select ComputerName -ExpandProperty ComputerName | sort
$session = New-PSSession -Credential $AdminCred  -ComputerName $Need2Boot
$n2b = invoke-Command -Session $session{
Get-Date -Displayhint time | Select-Object -ExpandProperty DateTime -ErrorAction SilentlyContinue
Hostname}
$n2b
